...
network *parse_network_cfg(char *filename)
{
	...
	/*
	 * Partition 3
	 * Case if index belongs to partition_point
	 * 1) if index not equals to TEE starting point --> divide the number of inputs by 2
 	 * 2) if index not equals to TEE ending point --> divide the number of outputs by 2
	 */
	
	if ( partition_point1< count && count <= partition_point2 ){
    		if( count !=partition_point1+1 ) l.inputs = (l.inputs+1)/2;
    		if( count !=partition_point2 ) l.outputs = (l.outputs+1)/2;
	}
